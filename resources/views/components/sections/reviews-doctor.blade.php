<section class="stock-items-sections section">
    <div class="container">
        ...
        <div class="swiper reviews-slider mt-5 max-[768px]:mt-4">
            <div class="swiper-wrapper">
                @forelse($data as $item)
                    <div class="swiper-slide">
                        <div class="stock-item">
                            <h3 class="stock-item__title">{!! $item->title !!}</h3>
                            <p class="stock-item__text text-neutral-600 ">
                                {{ \Illuminate\Support\Str::limit(strip_tags($item->description), 220, '...') }}</p>
                                    ...
                                    @for ($i = 1; $i <= $item->rating; $i++)
                                        <span class="star">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                class="" viewBox="0 0 24 24" stroke-width="1" stroke="#FFCC00"
                                                stroke-linecap="round"
                                                fill='{{ $i <= $item->rating ? '#FFCC00' : 'none' }}'
